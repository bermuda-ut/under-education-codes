...
#      Version: 0.0.1
#   LastChange: 2016-04-06 16:59:47
=============================================================================*/
#include <stdlib.h>
#include <assert.h>
...
Graph new_graph(int order) {
    ...
    // malloc check
    if(!(graphNew->vertices)) {
        fprintf(stderr, "Failed to allocate memory for graph vertices.\n");
        exit(EXIT_FAILURE);
    }
...
void free_graph(Graph graph) {
    ...
    for(i = 0; i < graph->order; i++) {
        ...
        // free edges - both in and out
        // leave the data(vertex) alone, handled later.
        free_list(current->in);
        free_list(current->out);

        // free label - array, NOT list
        free(current->label);
    }
